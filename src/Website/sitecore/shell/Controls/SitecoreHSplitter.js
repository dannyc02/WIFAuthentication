...
scHSplitter.prototype.mouseUp = function(tag, evt, id, target, nopost) {
  if (this.dragging) {
    ...
    if (ctl != null) {
      var prev = scForm.browser.getPreviousSibling(ctl.parentNode).childNodes[0];
      var next = scForm.browser.getNextSibling(ctl.parentNode).childNodes[0];

      var top = prev.offsetHeight;
      var bottom = next.offsetHeight;

      if (target == "top") {
        prev.style.height = top + this.delta + "px";
      }
