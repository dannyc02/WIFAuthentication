...
scSitecore.prototype.onKeyDown = function(evt) {
  ...
  if (evt != null) {
    if (evt.keyCode == 145 && evt.altKey && evt.shiftKey && evt.ctrlKey) {
...
scSitecore.prototype.onLoad = function() {
  ...
  this.browser.attachEvent(document, "onkeypress", function(evt) { evt = evt || window.event; if (evt.keyCode == 0 && evt.ctrlKey) { scForm.browser.clearEvent(evt, true, false); } });
...
scSitecore.prototype.invoke = function(method) {
  ...
  if (method.substring(0, 11) == "javascript:") {
    ...
    var result = eval(method.substr(11).replace(/&quot;/gi, "'"));
...
scSitecore.prototype.postEvent = function (tag, evt, parameters) {
  ...
  if (parameters != null && parameters.substring(0, 11) == "javascript:") {
    result = eval(parameters.substr(11).replace(/&quot;/gi, "'"));
  }
  else {
  ...
  switch (name) {
    ...
    case "Input":
      ...
      while (true) {
        ...
        alert("The length of the value is too long.\n\nPlease specify a value of less than " + maxlength + " characters.");
      }
