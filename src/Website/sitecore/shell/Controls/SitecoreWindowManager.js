function scWindowManager() {
  ...
  if (agent.indexOf("msie") >= 0 && agent.indexOf("opera") < 0) {
    this.browser = new scWindowManagerIE();
  }
  else {
...
scWindowManager.prototype.close = function (id) {
  ...
  if (win != null && this.closeFrame(cw)) {
    this.hide(id);
...
scWindowManager.prototype.closeFrame = function(win) {
  var result = true;
  
  if (win.scForm != null && win.scBeforeUnload != null) {
    if (win.scForm.modified) {
      var result = win.scForm.browser.showModalDialog("/sitecore/shell/default.aspx?xmlcontrol=YesNoCancel&te=" + encodeURIComponent(scForm.translate("Do you want to save the changes to the item?")), new Array(win), "dialogWidth:300px;dialogHeight:136px;help:no;scroll:auto;resizable:yes;status:no;center:yes");
  ...
  if (result) {
    for(var n = 0; n < win.frames.length && result; n++) {
      result = this.closeFrame(win.frames[n]);
    }
  }
...
scWindowManager.prototype.closeWindow = function (win, form) {
  if (win == null && form != null) {
    ...
    while (win != null && typeof (win.scWin) == "undefined") {
      win = win.parent;
    }
  }
